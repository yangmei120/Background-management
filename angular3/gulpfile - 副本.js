...
gulp.task('template', function() {
	console.log('开始copy template模版文件...'.cyan);
	gulp.src(config.src + '/template/**')
	.pipe(gulp.dest(config.output + '/template'));
});
...
gulp.task('dev', function() {
	runSequence('clean', 'replace:dev', ['handle', 'data', 'template']);
});

// beta环境
gulp.task('beta', function() {
	runSequence('clean', 'replace:beta', ['handle', 'template']);
});

// 线上环境
gulp.task('build', function() {
	runSequence('clean', 'replace:prod', ['handle', 'template']);
});
